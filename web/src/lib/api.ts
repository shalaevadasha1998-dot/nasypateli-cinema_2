...
function demoAction(action:string,payload:Record<string,unknown>){
  switch(action){
    ...
    case 'birth-creature':
    case 'participant-birth-v2': return mutate(s=>({...s,creature:{...s.creature,born:true,bornAt:new Date().toISOString(),name:String(payload.name||'Животина'),crumbs:Math.max(3,s.creature.crumbs)}}))
