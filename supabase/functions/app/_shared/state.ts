...
export async function buildEventState(db:any,event:any,opts:{includeActuals?:boolean;includePrivateOutputs?:boolean}={}){
  const [ideaF,selIdea,movieF,selMovie,preds,leaders,outputs,paid]=await Promise.all([
  ...
  for(const r of [ideaF,selIdea,movieF,selMovie,preds,leaders,outputs,paid]) if(r.error) throw r.error
